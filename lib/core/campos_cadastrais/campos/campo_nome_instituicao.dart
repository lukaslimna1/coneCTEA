...
/// Campo de formulário padronizado para Nome da Instituição.
class CampoNomeInstituicao extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final String labelTexto = '${label ?? 'Nome da instituição'}${requiredField ? ' *' : ''}';

    return DsInput(
      label: labelTexto,
      controller: controller,
      enabled: enabled,
      hint: hint ?? 'Digite o nome da instituição',
      ...
      validator: validator ??
          (requiredField
              ? (value) => ValidadoresCadastrais.campoObrigatorio(
                    value,
                    mensagem: 'Nome da instituição é obrigatório',
                  )
              : null),
    );
  }
