...
--
-- MIGRATION: 20260628110000_create_cpf_change_request_rpc_v2_review_data.sql
-- OBJETIVO:
--   - Criar a função privada v2 private.conectea_create_cpf_change_request_v2.
--   - Criar a função wrapper pública v2 public.conectea_create_cpf_change_request_v2.
--   - Permitir a inserção na mesma transação atômica dos dados claros de CPF
--     e do ID do documento de identidade na tabela private.account_change_review_data.
--
-- PRIVACIDADE E SEGURANÇA:
--   - As funções são SECURITY DEFINER e restritas estritamente a service_role.
...
--   - Os CPFs puros em memória e o document_file_id são mantidos isolados na
--     tabela privada private.account_change_review_data.
-- =========================================================================

-- ─────────────────────────────────────────────────────────────────────────
-- 1. RPC PRIVADA V2 NO SCHEMA PRIVATE
-- ─────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  ELSE
    ...
    IF v_doc_file_id !~ '^[A-Za-z0-9_-]{10,256}$' THEN
      RETURN jsonb_build_object('success', false, 'error_code', 'invalid_request');
    END IF;
...
COMMENT ON FUNCTION private.conectea_create_cpf_change_request_v2 IS
  'Função SQL/PLPGSQL privada restrita a service_role para criação transacional de solicitações de alteração de CPF (V2) com gravação na tabela de review privada na mesma transação.';

-- ─────────────────────────────────────────────────────────────────────────
-- 2. WRAPPER PÚBLICO V2 NO SCHEMA PUBLIC
-- ─────────────────────────────────────────────────────────────────────────

CREATE OR REPLACE FUNCTION public.conectea_create_cpf_change_request_v2(
  p_user_id uuid,
  p_new_cpf_clear text,
  p_new_cpf_hmac text,
  p_justification text,
...
AS $$
BEGIN
  RETURN private.conectea_create_cpf_change_request_v2(
    p_user_id => p_user_id,
...
-- Revogação total de privilégios públicos padrão no wrapper público v2
REVOKE ALL ON FUNCTION public.conectea_create_cpf_change_request_v2(
  uuid, text, text, text, text, text, text, text, integer, text
) FROM PUBLIC, anon, authenticated;

-- Concessão de execução exclusiva para o service_role
GRANT EXECUTE ON FUNCTION public.conectea_create_cpf_change_request_v2(
  uuid, text, text, text, text, text, text, text, integer, text
) TO service_role;

COMMENT ON FUNCTION public.conectea_create_cpf_change_request_v2 IS
  'Wrapper pública restrita a service_role que intermedia e repassa os parâmetros para a RPC privada v2 conectea_create_cpf_change_request_v2, evitando exposição do schema private.';
