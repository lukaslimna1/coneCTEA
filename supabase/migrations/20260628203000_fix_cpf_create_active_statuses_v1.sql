-- =========================================================================
-- ConeCTEA — Remoção de application_failed da Criação CPF Ativa
--
-- MIGRATION: 20260628203000_fix_cpf_create_active_statuses_v1.sql
-- OBJETIVO:
--   - Atualizar private.conectea_create_cpf_change_request_v2
--     removendo application_failed da checagem active_request_exists.
--   - Revogar acesso PUBLIC das funções de cleanup se aplicável.
-- =========================================================================
...
COMMENT ON FUNCTION private.conectea_create_cpf_change_request_v2 IS
  'Função SQL/PLPGSQL privada restrita a service_role para criação transacional de solicitações de alteração de CPF (V2). Modificada para não considerar application_failed como status ativo impeditivo de novas criações.';

-- Revogação do PUBLIC de clear status change trigger se aplicável e existente.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'conectea_clear_cpf_review_data_on_status_change_v1' AND pronamespace = 'private'::regnamespace) THEN
    EXECUTE 'REVOKE ALL ON FUNCTION private.conectea_clear_cpf_review_data_on_status_change_v1() FROM PUBLIC, anon';
  END IF;
END
$$;
