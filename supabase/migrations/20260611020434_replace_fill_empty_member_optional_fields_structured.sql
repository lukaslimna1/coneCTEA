-- ConeCTEA — Migration: RPC para Preenchimento Atômico de Campos Opcionais Vazios (V2)
-- MIGRATION: 20260611000000_replace_fill_empty_member_optional_fields_structured.sql

-- NOTA: A V1 (conectea_fill_empty_member_optional_fields) é preservada intacta para retrocompatibilidade.
-- Esta migration cria apenas a V2 com retorno de campos estruturados.
...
DECLARE
  ...
  -- Campos compostos legados para dual-write
...
BEGIN
  ...
  -- Dual-write do responsável
  IF v_new_responsible_person_name IS NOT NULL AND btrim(v_new_responsible_person_name) != '' THEN
    IF v_new_responsible_phone IS NOT NULL AND btrim(v_new_responsible_phone) != '' THEN
      v_new_responsible_name := v_new_responsible_person_name || ' - ' || v_new_responsible_phone;
    ELSE
      v_new_responsible_name := v_new_responsible_person_name;
    END IF;
  ELSE
  ...
  -- Dual-write da emergência
  IF v_new_emergency_person_name IS NOT NULL AND btrim(v_new_emergency_person_name) != '' THEN
    IF v_new_emergency_phone IS NOT NULL AND btrim(v_new_emergency_phone) != '' THEN
      v_new_emergency_contact := v_new_emergency_person_name || ' - ' || v_new_emergency_phone;
    ELSE
      v_new_emergency_contact := v_new_emergency_person_name;
    END IF;
  ELSE
...
) IS 'V2: Preenche de forma atômica e segura somente os campos opcionais vazios na tabela members, usando contatos estruturados e com dual-write legados.';
