-- =========================================================================
-- ConeCTEA — Refinamento de Diagnósticos na Criação de CPF de Dependente
--
-- MIGRATION: 20260723043000_refine_dependent_cpf_create_error_codes_v1.sql
-- OBJETIVO:
--   - Refinar os códigos de erro retornados pela RPC privada
--     `private.conectea_create_dependent_cpf_change_request_v1` para regras
--     de negócio de status do membro, CPF idêntico ao atual e conflito de CPF.
--   - Substituir `invalid_request` por:
--       - `member_not_active`: quando status do membro for inativo/não aprovado.
--       - `same_current_cpf`: quando o CPF solicitado for idêntico ao atual do dependente.
--       - `unavailable`: quando o CPF solicitado já estiver cadastrado para outro dependente
--         (protegendo a privacidade sem revelar a existência de cadastro de terceiros).
--   - Manter `invalid_request` para validações técnicas de formato e integridade.
-- =========================================================================
...
BEGIN
  ...
  -- 8. Verificar se o CPF solicitado já existe cadastrado em outros dependentes (members) (Retorna código seguro unavailable)
  SELECT EXISTS (
    SELECT 1 
    FROM public.members 
    WHERE id <> p_member_id
...
COMMENT ON FUNCTION private.conectea_create_dependent_cpf_change_request_v1(
  ...
  'Função PL/PGSQL privada restrita a service_role para criação transacional de solicitações de alteração de CPF de dependente. Refinada para retornar member_not_active, same_current_cpf e unavailable em vez de invalid_request genérico em erros de regra de negócio.';
