...
export async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const { data: rpcData, error: rpcError } = await supabaseAdmin.rpc(
      "conectea_submit_cpf_correction_v1",
      {
    ...
    if (success === true) {
      ...
      const mappedError = ["cpf_conflict", "expired", "invalid_cpf", "forbidden", "invalid_status", "not_found", "cpf_unchanged", "unauthenticated"].includes(error_code)
        ? error_code
        : "unavailable";
