# Documentação do ConeCTEA

**App:** 0.6.0-dev
**Documentação:** 4.3.0
**Status:** Desenvolvimento

---

## Objetivo

Esta pasta concentra a documentação técnica, operacional, visual, de governança, QA, release e integrações do ConeCTEA.

O `README.md` da raiz apresenta o projeto.
O `DOCTecnico.md` da raiz mantém a visão técnica principal.
A pasta `docs/` guarda os documentos detalhados por tema.

---

## Índice

### Histórico
- [Histórico de Frentes](historico/FRENTES.md)

### Arquitetura
- [Arquitetura Geral](arquitetura/ARQUITETURA_GERAL.md)
- [Home Unificada](arquitetura/HOME_UNIFICADA.md)
- [Central do Usuário](arquitetura/ACCOUNT.md)
- [Auth](arquitetura/AUTH.md)
- [Requests](arquitetura/REQUESTS.md)
- [Carteirinha Digital](arquitetura/CARTEIRINHA_DIGITAL.md)
- [Admin](arquitetura/ADMIN.md)
- [Notificações](arquitetura/NOTIFICACOES.md)

### Design System
- [Design System](design-system/DESIGN_SYSTEM.md)
- [Status Visuais](design-system/STATUS_VISUAIS.md)
- [Avatares](design-system/AVATARES.md)
- [Componentes Premium](design-system/COMPONENTES_PREMIUM.md)

### Governança
- [Governança de Dados](governanca/GOVERNANCA_DADOS.md)
- [LGPD](governanca/LGPD.md)
- [Segurança](governanca/SEGURANCA.md)
- [Logs e Privacidade](governanca/LOGS_E_PRIVACIDADE.md)
- [Documentos Sensíveis](governanca/DOCUMENTOS_SENSIVEIS.md)

### QA
- [QA Android](qa/QA_ANDROID.md)
- [Matriz de Dispositivos](qa/MATRIZ_DISPOSITIVOS.md)
- [Checklist de Testes](qa/CHECKLIST_TESTES.md)
- [Bugs Conhecidos](qa/BUGS_CONHECIDOS.md)

### Operação
- [Fluxos Admin](operacao/FLUXOS_ADMIN.md)
- [Fluxos Usuário](operacao/FLUXOS_USUARIO.md)
- [Status e Regras](operacao/STATUS_E_REGRAS.md)
- [Suporte](operacao/SUPORTE.md)

### Release
- [Versionamento](release/VERSIONAMENTO.md)
- [Checklist de Release](release/CHECKLIST_RELEASE.md)
- [Google Play](release/GOOGLE_PLAY.md)

### Integrações
- [Supabase](integracoes/SUPABASE.md)
- [Google Drive + GAS](integracoes/GOOGLE_DRIVE_GAS.md)
- [OneSignal](integracoes/ONESIGNAL.md)
- [Edge Functions](integracoes/EDGE_FUNCTIONS.md)
